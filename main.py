...
try:
    ...
    logger.warning("EasyOCR not available. Photo OCR feature will be disabled.")
...
class CoffeeBot:
    def __init__(self, token: str):
        ...
        # Initialize OCR reader lazily (only when needed, not during startup)
        # This prevents build timeout - models will be downloaded on first use
            ...
            if OCR_AVAILABLE and self.ocr_reader:
                try:
                    ...
                    if extracted_data:
                        ...
                        await update.message.reply_text("⚠️ Could not extract information from the photo. Please fill the form manually.")
                except Exception as e:
                    logger.error(f"OCR error: {e}", exc_info=True)
                    await update.message.reply_text("⚠️ Error processing photo. Please fill the form manually.")
            else:
                await update.message.reply_text("⚠️ OCR is not available. Please fill the form manually.")
